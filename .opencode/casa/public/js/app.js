...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  pollStatus();
  loadDashboardCharts();
  // Restaurar som de notificação
  const selSom = document.getElementById("sel-som");
  if (selSom) selSom.value = _somNotificacao;
});
...
const NAV_TITLES = {
  ...
  clientes:"Clientes", logs:"Logs", enviar:"Enviar Mensagem", aparencia:"🎨 Aparência", suporte:"Licença & Suporte",
  atendimento:"👤 Atendimento", tts:"🔊 Voz do Bot", pesquisa:"📋 Pesquisa"
};
...
document.querySelectorAll(".nb").forEach(b => {
  b.addEventListener("click", () => {
    ...
    if (tab==="tts") loadTTSConfig();
    if (tab==="pesquisa") loadPesquisaConfig();
    closeSB();
  });
});
...
function initSocket() {
  ...
  socket.on("comprovante_verificado", d => {
    showToast(`Comprovante ${d.status === "verificado" ? "verificado" : "rejeitado"}!`, d.status === "verificado" ? "ok" : "er");
  });
  ...
  socket.on("pesquisa_satisfacao", d => {
    const media = d.media || 0;
    const e2 = "⭐".repeat(Math.round(media));
    addFeed({ numero:d.numero, mensagem:`${e2} Pesquisa: ${media}/5`, tipo:"entrada" });
    showToast(`Pesquisa recebida! Média: ${media}/5`, "ok");
  });
...
function showToast(msg, type="ok") {
  ...
  toast.style.cssText = `
    ...
    background: ${type === "ok" ? "#1bcc6f" : "#ff3351"};
  `;
...
const SONS_NOTIFICACAO = {
  ...
  suave: () => {
    ...
    o.frequency.setValueAtTime(523,ctx.currentTime);o.frequency.linearRampToValueAtTime(784,ctx.currentTime+.2);
    g.gain.setValueAtTime(0,ctx.currentTime);g.gain.linearRampToValueAtTime(.5,ctx.currentTime+.05);g.gain.linearRampToValueAtTime(0,ctx.currentTime+.4);
  ...
  urgente: () => {
    ...
    for(let i=0;i<4;i++){
      const o=ctx.createOscillator(),g=ctx.createGain();o.type="square";o.connect(g);g.connect(ctx.destination);
      o.frequency.value=880;g.gain.setValueAtTime(0,ctx.currentTime+i*.15);g.gain.linearRampToValueAtTime(.8,ctx.currentTime+i*.15+.01);g.gain.linearRampToValueAtTime(0,ctx.currentTime+i*.15+.12);
      o.start(ctx.currentTime+i*.15);o.stop(ctx.currentTime+i*.15+.12);
    }
  },
  campainha: () => {
    const ctx=new(window.AudioContext||window.webkitAudioContext)();
    const n=(f,t,d)=>{const o=ctx.createOscillator(),g=ctx.createGain();o.type="sine";o.connect(g);g.connect(ctx.destination);o.frequency.value=f;g.gain.setValueAtTime(0,ctx.currentTime+t);g.gain.linearRampToValueAtTime(.6,ctx.currentTime+t+.02);g.gain.exponentialRampToValueAtTime(.01,ctx.currentTime+t+d);o.start(ctx.currentTime+t);o.stop(ctx.currentTime+t+d);};
    n(659,0,.3);n(784,.3,.3);n(659,.6,.3);
  },
  nenhum: () => {}
};

let _somNotificacao = localStorage.getItem("som_notificacao") || "classico";

function bipAlto() {
  try {
    const fn = SONS_NOTIFICACAO[_somNotificacao] || SONS_NOTIFICACAO.classico;
    fn();
    if (_somNotificacao !== "nenhum") { setTimeout(fn, 700); setTimeout(fn, 1400); }
  } catch(_){}
}
...
function mkCardPedido(p) {
    ...
    if (p.comprovantes && p.comprovantes.length > 0) {
        comprovantesHtml = `<div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--brd)"><div style="font-size:.74rem;color:var(--mu);margin-bottom:6px">📸 Comprovantes Pix:</div>`;
        p.comprovantes.forEach((c, i) => {
            const caminho = `/logs/comprovantes/${c.filename}`;
            const statusClass = c.status || "pendente";
            const statusLabel = c.status === "verificado" ? "✅ Verificado" : c.status === "rejeitado" ? "❌ Rejeitado" : "⏳ Pendente";
            const borderCor = c.status === "verificado" ? "var(--gr)" : c.status === "rejeitado" ? "var(--rd)" : "var(--am)";
            
            comprovantesHtml += `
            <div class="comprovante-item" style="border-left:3px solid ${borderCor}">
              <div style="display:flex;gap:10px;align-items:flex-start;flex-wrap:wrap">
                <div style="flex-shrink:0;cursor:pointer" onclick="abrirComprovanteModal('${caminho}')">
                  <img src="${caminho}" alt="Comprovante #${i+1}" style="width:120px;height:160px;object-fit:cover;border-radius:6px;border:1px solid var(--brd);transition:transform .2s" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'"/>
                </div>
                <div style="flex:1;min-width:140px">
                  <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">
                    <span style="font-weight:700;font-size:.8rem">Comprovante #${i+1}</span>
                    <span class="comprovante-status ${statusClass}">${statusLabel}</span>
                  </div>
                  ${c.valorOCR ? `<div class="comprovante-ocr">💰 <strong>R$ ${esc(c.valorOCR)}</strong></div>` : ""}
                  ${c.dataOCR ? `<div class="comprovante-ocr">📅 ${esc(c.dataOCR)}</div>` : ""}
                  ${c.chaveOCR ? `<div class="comprovante-ocr">🔑 ${esc(c.chaveOCR)}</div>` : ""}
                  ${c.observacao ? `<div class="comprovante-ocr">📝 ${esc(c.observacao)}</div>` : ""}
                  <div class="comprovante-acoes" style="margin-top:8px">
                    <button class="btn btg btsm" onclick="verificarComprovante('${esc(p.numero)}', '${c.filename}', 'verificado')">✅ Verificar</button>
                    <button class="btn btd btsm" onclick="rejeitarComprovante('${esc(p.numero)}', '${c.filename}')">❌ Rejeitar</button>
                    <a href="${caminho}" target="_blank" class="btn bto btsm">🔍 Ampliar</a>
                  </div>
                </div>
              </div>
            </div>`;
        });
        comprovantesHtml += `</div>`;
    }
    ...
    return `<div class="pc" id="${pid}"><span class="pc-tag">NOVO</span>
      <div class="pm">
        ...
        <div class="pa">${maps}<button class="btn btd btsm" onclick='doPrint(JSON.parse(this.dataset.p))' data-p="${pj}">🖨️</button><button class="btn btp btsm" onclick="sairEntrega('${esc(p.numero)}')">🛵 Saiu</button><button class="btn btg btsm" onclick="entregar('${esc(p.numero)}')">✅ Entregue</button></div>
      </div>
      <div class="pb"><div class="pi">🛒 ${esc(p.itens||"—")}</div><div class="pd">${end}${comp}${ref}${tel}${obs}${troco}<div>💳 ${esc(p.pagamento||"—")}</div>${comprovantesHtml}</div></div>
    </div>`;
}

// Funções para verificar/rejeitar comprovantes
window.verificarComprovante = async function(numero, filename, status) {
  const obs = prompt("Observação (opcional):");
  const res = await api("POST", "/api/comprovantes/verificar", { numero, filename, status, observacao: obs });
  if (res?.ok) {
    showToast("Comprovante verificado com sucesso!", "ok");
  } else {
    showToast("Erro ao verificar comprovante: " + (res?.erro || "Desconhecido"), "er");
  }
};

window.rejeitarComprovante = async function(numero, filename) {
  const obs = prompt("Motivo da rejeição (obrigatório):");
  if (!obs) {
    showToast("Motivo é obrigatório!", "er");
    return;
  }
  const res = await api("POST", "/api/comprovantes/verificar", { numero, filename, status: "rejeitado", observacao: obs });
  if (res?.ok) {
    showToast("Comprovante rejeitado!", "ok");
  } else {
    showToast("Erro ao rejeitar comprovante: " + (res?.erro || "Desconhecido"), "er");
  }
};

window.abrirComprovanteModal = function(src) {
  const modal = document.createElement("div");
  modal.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;display:flex;align-items:center;justify-content:center;cursor:pointer;animation:ti .2s";
  modal.onclick = () => modal.remove();
  modal.innerHTML = `
    <div style="position:relative;max-width:90vw;max-height:90vh" onclick="event.stopPropagation()">
      <img src="${src}" style="max-width:90vw;max-height:85vh;border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,.6)"/>
      <button onclick="this.parentElement.parentElement.remove()" style="position:absolute;top:-12px;right:-12px;width:36px;height:36px;border-radius:50%;background:var(--rd);color:#fff;border:none;font-size:1.1rem;cursor:pointer;box-shadow:0 4px 12px rgba(0,0,0,.4)">✕</button>
      <a href="${src}" target="_blank" style="position:absolute;bottom:12px;right:12px;padding:8px 14px;background:var(--ac);color:#fff;border-radius:8px;font-size:.8rem;text-decoration:none;font-weight:600">🔗 Abrir em nova aba</a>
    </div>`;
...
window.filtrarPedidos = function(busca) {
  const filtroStatus = document.getElementById("filtro-status-ped")?.value || "";
  busca = (busca || "").toLowerCase().trim();
  let filtrados = _pedidosAtuais;
  
  if (busca) {
    filtrados = filtrados.filter(p => {
      const num = ((p.numPedido || "") + " " + (p.numero || "")).toLowerCase();
      const itens = (p.itens || "").toLowerCase();
      const end = (p.endereco || "").toLowerCase();
      const tel = (p.telefone || "").toLowerCase();
      return num.includes(busca) || itens.includes(busca) || end.includes(busca) || tel.includes(busca);
    });
  }
  
  const html = filtrados.length ? filtrados.map(p => mkCardPedido(p)).join("") : '<div class="fe">Nenhum pedido encontrado.</div>';
  setEl("lista-ped", "html", html);
};
window.sairEntrega=async num=>{ await api("POST",`/api/pedidos/${encodeURIComponent(num)}/sair`); showToast("🛵 Pedido saiu para entrega!","ok"); };
window.entregar=async num=>{ await api("POST",`/api/pedidos/${encodeURIComponent(num)}/entregar`); showToast("✅ Pedido entregue!","ok"); };
...
function mostrarPrint(p,auto){
  ...
  get("pr-i").textContent=p.pagamento||"—";
  const prr=get("pr-prod-row"); if(p.producao){get("pr-prod").textContent=p.producao;prr.style.display="flex";}else prr.style.display="none";
  get("pr-p").textContent=p.pagamentoTipo||p.pagamento||"—";
...
function doPrint(p){
  ...
  // VIA 1: Cupom de PAGAMENTO (completo com valores)
  const viaPagamento = `<div style="padding-top:7px;">
    <h1>💰 ${esc(empresa)}</h1>
    <div class="s">PEDIDO #<strong>${p.numPedido||"—"}</strong> • ${hora}</div>
    <div style="padding:10px 0;font-size:14px;font-weight:bold;white-space:pre-wrap;line-height:1.4;text-align:center">${esc(p.pagamento||p.itens||"—")}</div>
    <div class="f">CUPOM DE PAGAMENTO</div>
  </div>`;

  // VIA 2: Cupom de PRODUÇÃO (itens para cozinha)
  const viaProducao = `<div style="page-break-before:always;padding-top:7px;border-top:2px dashed #333;">
    <h1 style="background:#000;color:#fff;padding:5px">👨‍🍳 PRODUÇÃO</h1>
    <div class="s">PEDIDO #<strong>${p.numPedido||"—"}</strong> • ${hora}</div>
    <div class="r"><span class="l">📱 Cliente</span><span class="v">${esc(num)}</span></div>
    <div style="padding:10px 0;font-size:16px;font-weight:bold;white-space:pre-wrap;line-height:1.4">${esc(p.producao||p.itens||"—")}</div>
    ${p.observacao?`<div style="margin-top:10px;padding:8px;background:#eee;border-radius:4px"><strong>OBS:</strong> ${esc(p.observacao)}</div>`:""}
    <div class="f">CUPOM DE PRODUÇÃO — SEM VALORES</div>
  </div>`;

  const html=`<!DOCTYPE html><html><head><meta charset="UTF-8"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:13px;padding:9px;width:300px;color:#111}h1{font-size:14px;font-weight:bold;text-align:center;border-bottom:2px solid #111;padding-bottom:5px;margin-bottom:4px}.s{text-align:center;font-size:11px;color:#555;margin-bottom:9px}.r{display:flex;justify-content:space-between;gap:7px;padding:4px 0;border-bottom:1px dashed #ccc;align-items:flex-start}.r:last-child{border:none}.l{color:#555;font-size:11px;flex-shrink:0;min-width:62px}.v{font-weight:bold;font-size:12px;text-align:right;word-break:break-word}.f{margin-top:9px;text-align:center;font-size:10px;color:#888;border-top:1px dashed #ccc;padding-top:6px}@media print{@page{size:80mm auto;margin:0}body{padding:3px;width:80mm}}</style></head><body>${viaPagamento}${p.producao?viaProducao:""}</body></html>`;
...
// ── RELATÓRIO GLOBAL ──
let _chartRelFatDia = null;
let _chartRelFatHora = null;
let _chartRelPag = null;
let _chartRelSemana = null;
let _chartRelHora = null;

function setPeriodo(p) {
  const hoje = new Date();
  const ini = document.getElementById("rel-global-ini");
  const fim = document.getElementById("rel-global-fim");

  switch(p) {
    case "hoje":
      ini.value = hoje.toISOString().slice(0,10);
    ...
    case "30dias":
      ...
      ini.value = d30.toISOString().slice(0,10);
      fim.value = hoje.toISOString().slice(0,10);
      break;
    case "mes":
      ini.value = hoje.toISOString().slice(0,7) + "-01";
      fim.value = hoje.toISOString().slice(0,10);
      break;
    case "todos":
      ini.value = "2020-01-01";
      fim.value = hoje.toISOString().slice(0,10);
      break;
  }
}

document.getElementById("btn-gerar-relatorio")?.addEventListener("click", gerarRelatorioGlobal);

async function gerarRelatorioGlobal() {
  const ini = document.getElementById("rel-global-ini")?.value;
  const fim = document.getElementById("rel-global-fim")?.value;

  if (!ini || !fim) {
    toast("t-relatorio", "❌ Selecione o período!", false);
    return;
  }

  toast("t-relatorio", "⏳ Gerando relatório...", true);

  const r = await api("GET", `/api/relatorio-global?dataInicio=${ini}&dataFim=${fim}`);
  if (!r) {
    toast("t-relatorio", "❌ Erro ao gerar relatório!", false);
    return;
  }

  renderRelatorioGlobal(r);
  document.getElementById("relatorio-resultado").style.display = "block";
  toast("t-relatorio", "✅ Relatório gerado com sucesso!", true);
}

function renderRelatorioGlobal(r) {
  const isDark = document.documentElement.getAttribute("data-skin") !== "light";
  const textColor = isDark ? "#8892b8" : "#666";
  const gridColor = isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.08)";

  const custom = localStorage.getItem("dbCustomColors");
  let c = {};
  if (custom) { try { c = JSON.parse(custom); } catch(_) {} }
  const accentColor = c.accent || "#ff4f1f";
  const successColor = c.success || "#1bcc6f";
  const chartColor = c.chart || "#3d95ff";

  // Resumo Executivo
  const resumoEl = document.getElementById("rel-resumo");
  if (resumoEl) {
    resumoEl.innerHTML = `
      <div class="stat sg"><div style="font-size:1.1rem">📦</div><div class="sv">${r.resumo.totalPedidos}</div><div class="sl">Total Pedidos</div></div>
      <div class="stat sp"><div style="font-size:1.1rem">💰</div><div class="sv" style="font-size:1.1rem">R$ ${r.resumo.faturamentoTotal.toFixed(2).replace(".",",")}</div><div class="sl">Faturamento</div></div>
      <div class="stat so"><div style="font-size:1.1rem">🎯</div><div class="sv" style="font-size:1.1rem">R$ ${r.resumo.ticketMedio.toFixed(2).replace(".",",")}</div><div class="sl">Ticket Médio</div></div>
      <div class="stat sb2"><div style="font-size:1.1rem">👥</div><div class="sv">${r.resumo.clientesUnicos}</div><div class="sl">Clientes</div></div>
      <div class="stat sg"><div style="font-size:1.1rem">🔄</div><div class="sv">${r.resumo.clientesRecorrentes}</div><div class="sl">Recorrentes</div></div>
      <div class="stat sp"><div style="font-size:1.1rem">⭐</div><div class="sv">${r.resumo.mediaAvaliacao}/5</div><div class="sl">Avaliação</div></div>
      <div class="stat so"><div style="font-size:1.1rem">📋</div><div class="sv">${r.resumo.mediaPesquisa}/5</div><div class="sl">Pesquisa</div></div>
      <div class="stat sb2"><div style="font-size:1.1rem">⏰</div><div class="sv" style="font-size:.8rem">${r.resumo.horaPico}</div><div class="sl">Hora Pico</div></div>
    `;
  }

  // Faturamento por Dia
  const ctxFatDia = document.getElementById("chart-rel-fat-dia");
  if (ctxFatDia && r.financeiro.porDia.length) {
    if (_chartRelFatDia) _chartRelFatDia.destroy();
    const labels = r.financeiro.porDia.map(d => {
      const dt = new Date(d.data + "T12:00");
      return dt.toLocaleDateString("pt-BR", { day:"2-digit", month:"2-digit" });
    });
    const data = r.financeiro.porDia.map(d => d.total);
    _chartRelFatDia = new Chart(ctxFatDia, {
      type: "bar",
      data: {
        labels,
        datasets: [{
  ...
  const ctxFatHora = document.getElementById("chart-rel-fat-hora");
  if (ctxFatHora) {
    if (_chartRelFatHora) _chartRelFatHora.destroy();
    const labels = r.financeiro.porHora.map(h => h.hora);
    const data = r.financeiro.porHora.map(h => h.total);
    _chartRelFatHora = new Chart(ctxFatHora, {
      type: "line",
      data: {
        labels,
        datasets: [{
  ...
  if (rankEl) {
    if (r.produtos.ranking.length) {
      const maxQtd = r.produtos.ranking[0]?.qtd || 1;
      rankEl.innerHTML = r.produtos.ranking.map((p, i) => {
        ...
        return `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--brd)">
          <span style="font-family:var(--fh);font-weight:800;font-size:.9rem;min-width:24px;text-align:center;color:${i<3?"var(--ac)":"var(--mu)"}">${i===0?"🥇":i===1?"🥈":i===2?"🥉":"#"+(i+1)}</span>
          <span style="flex:1;font-size:.82rem">${esc(p.nome)}</span>
          <div style="width:80px;height:6px;background:var(--bg4);border-radius:3px;overflow:hidden">
            <div style="height:100%;width:${pct}%;background:${accentColor};border-radius:3px"></div>
          </div>
          <span style="font-size:.76rem;color:var(--mu);min-width:30px;text-align:right">${p.qtd}x</span>
        </div>`;
      }).join("");
    } else {
      rankEl.innerHTML = '<div class="fe">Sem dados.</div>';
    }
  }

  // Pagamentos
  const pagListEl = document.getElementById("rel-pag-lista");
  if (pagListEl) {
    pagListEl.innerHTML = r.pagamentos.map(p => `
      <div style="display:flex;justify-content:space-between;padding:4px 0;font-size:.82rem;border-bottom:1px solid var(--brd)">
        <span>${esc(p.tipo)}</span>
        <span>${p.qtd} (${p.percentual}%)</span>
      </div>
    `).join("");
  }

  const ctxPag = document.getElementById("chart-rel-pag");
  if (ctxPag && r.pagamentos.length) {
    if (_chartRelPag) _chartRelPag.destroy();
    const cores = [accentColor, successColor, chartColor, "#f59e0b", "#9d7aff", "#f43f5e"];
    _chartRelPag = new Chart(ctxPag, {
      type: "doughnut",
      data: {
        labels: r.pagamentos.map(p => p.tipo),
        datasets: [{
          data: r.pagamentos.map(p => p.qtd),
          backgroundColor: r.pagamentos.map((_, i) => cores[i % cores.length])
        }]
      },
      options: {
        responsive: true,
  ...
  // Clientes
  const cliStatsEl = document.getElementById("rel-clientes-stats");
  if (cliStatsEl) {
    cliStatsEl.innerHTML = `
      <div class="stat sg"><div style="font-size:1.1rem">👥</div><div class="sv">${r.clientes.total}</div><div class="sl">Total</div></div>
      <div class="stat so"><div style="font-size:1.1rem">🔄</div><div class="sv">${r.clientes.recorrentes}</div><div class="sl">Recorrentes</div></div>
      <div class="stat sp"><div style="font-size:1.1rem">📊</div><div class="sv">${r.clientes.taxaRetencao}%</div><div class="sl">Retenção</div></div>
    `;
  }

  const topCliEl = document.getElementById("rel-top-clientes");
  if (topCliEl && r.clientes.top.length) {
    topCliEl.innerHTML = `<div style="font-size:.82rem;font-weight:600;margin-bottom:8px">Top Clientes:</div>` +
      r.clientes.top.map((c, i) => `
        <div style="display:flex;align-items:center;gap:8px;padding:5px 0;font-size:.82rem;border-bottom:1px solid var(--brd)">
          <span style="font-weight:700;color:var(--ac);min-width:20px">${i+1}.</span>
          <span style="flex:1">${esc(c.numero)}</span>
          <span style="color:var(--mu)">${c.pedidos} pedidos</span>
        </div>
      `).join("");
  }

  // Avaliações
  const avalStatsEl = document.getElementById("rel-aval-stats");
  if (avalStatsEl) {
    const estrelas = "⭐".repeat(Math.round(r.avaliacoes.media));
    avalStatsEl.innerHTML = `
      <div style="font-size:2rem;margin-bottom:6px">${estrelas}</div>
      <div style="font-family:var(--fh);font-size:2.5rem;font-weight:800;color:var(--ac)">${r.avaliacoes.media}/5</div>
      <div style="color:var(--mu);font-size:.82rem">${r.avaliacoes.total} avaliações</div>
    `;
  }

  const avalDistEl = document.getElementById("rel-aval-dist");
  if (avalDistEl) {
    const totalAval = r.avaliacoes.total || 1;
    const cores = ["#ff3351", "#ff6e3f", "#f59e0b", "#1bcc6f", "#0ea5e9"];
    avalDistEl.innerHTML = [5,4,3,2,1].map(n => {
      const qtd = r.avaliacoes.distribuicao?.[n] || 0;
      const pct = Math.round((qtd / totalAval) * 100);
      return `<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
        <span style="font-size:.76rem;min-width:20px">${n}⭐</span>
        <div style="flex:1;height:14px;background:var(--bg4);border-radius:3px;overflow:hidden">
          <div style="height:100%;width:${pct}%;background:${cores[n-1]};border-radius:3px"></div>
        </div>
        <span style="font-size:.72rem;min-width:25px;text-align:right">${qtd}</span>
      </div>`;
    }).join("");
  }

  const pesqStatsEl = document.getElementById("rel-pesq-stats");
  if (pesqStatsEl) {
    pesqStatsEl.innerHTML = `
      <div style="font-family:var(--fh);font-size:2rem;font-weight:800;color:var(--ac)">${r.avaliacoes.mediaPesquisaDetalhada}/5</div>
      <div style="color:var(--mu);font-size:.82rem">${r.avaliacoes.pesquisasTotal} pesquisas detalhadas</div>
    `;
  }

  // Pedidos por Dia da Semana
  const ctxSemana = document.getElementById("chart-rel-semana");
  if (ctxSemana) {
    if (_chartRelSemana) _chartRelSemana.destroy();
    const labels = Object.keys(r.operacional.porDiaSemana);
    const data = Object.values(r.operacional.porDiaSemana);
    _chartRelSemana = new Chart(ctxSemana, {
      type: "bar",
      data: {
        labels,
        datasets: [{
          label: "Pedidos",
          data,
          backgroundColor: labels.map((_, i) => i === 0 || i === 6 ? accentColor : chartColor),
          borderRadius: 4
        }]
      },
      options: {
  ...
  // Pedidos por Hora
  const ctxHora = document.getElementById("chart-rel-hora");
  if (ctxHora) {
    if (_chartRelHora) _chartRelHora.destroy();
    const labels = r.operacional.porHora.map(h => h.hora);
    const data = r.operacional.porHora.map(h => h.qtd);
    _chartRelHora = new Chart(ctxHora, {
      type: "bar",
      data: {
        labels,
        datasets: [{
  ...
  // Configurações
  const cfgEl = document.getElementById("rel-config");
  if (cfgEl) {
    const cfg = r.configuracoes;
    cfgEl.innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:.82rem">
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>Empresa:</strong> ${esc(cfg.empresa)}</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>Telefone:</strong> ${esc(cfg.telefone)}</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>Endereço:</strong> ${esc(cfg.endereco)}</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>Horário:</strong> ${esc(cfg.horario)}</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>Taxa Entrega:</strong> ${esc(cfg.taxaEntrega)}</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>Tempo Entrega:</strong> ${esc(cfg.tempoEntrega)}</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>Pedido Mínimo:</strong> ${esc(cfg.pedidoMinimo)}</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>Pagamentos:</strong> ${esc(cfg.pagamentos)}</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>Produtos:</strong> ${cfg.totalProdutos} em ${cfg.totalCategorias} categorias</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>IA:</strong> ${cfg.iaAtiva ? "✅ Ativa" : "❌ Inativa"}</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>Pesquisa:</strong> ${cfg.pesquisaAtiva ? "✅ Ativa" : "❌ Inativa"}</div>
        <div style="padding:8px;background:var(--bg3);border-radius:6px"><strong>TTS/Voz:</strong> ${cfg.ttsAtivo ? "✅ Ativo" : "❌ Inativo"}</div>
      </div>
    `;
  }

  // Pedidos Detalhados
  const pedEl = document.getElementById("rel-pedidos-lista");
  if (pedEl) {
    if (r.pedidosDetalhados.length) {
      pedEl.innerHTML = r.pedidosDetalhados.map(p => {
        const aval = p.avaliacao ? "⭐".repeat(Math.round(p.avaliacao)) : "—";
        return `<div style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--bg3);border-radius:6px;margin-bottom:4px;font-size:.82rem">
          <span style="font-family:var(--fh);font-weight:800;color:var(--ac);min-width:50px">#${esc(p.numPedido)}</span>
          <span style="color:var(--mu);min-width:70px">${p.data} ${p.hora||""}</span>
          <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc((p.itens||"").slice(0,50))}</span>
          <span style="font-weight:600;min-width:60px;text-align:right">R$ ${(p.total||0).toFixed(2).replace(".",",")}</span>
          <span style="min-width:30px">${aval}</span>
        </div>`;
      }).join("");
    } else {
      pedEl.innerHTML = '<div class="fe">Nenhum pedido no período.</div>';
    }
  }
...
// Imprimir relatório global
function imprimirRelatorio() {
  const el = document.getElementById("relatorio-resultado");
  if (!el) return;

  const empresa = document.getElementById("e-nome")?.value || "Delivery";
  const ini = document.getElementById("rel-global-ini")?.value || "";
  const fim = document.getElementById("rel-global-fim")?.value || "";
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    h2{font-size:13px;text-align:center;color:#666;margin-bottom:15px}
    h3{font-size:12px;margin:10px 0 5px;padding-bottom:3px;border-bottom:1px solid #ddd}
    .stats{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px}
    .stat{flex:1;min-width:100px;padding:8px;background:#f5f5f5;border-radius:6px;text-align:center}
  ...
  <h1>${esc(empresa)}</h1>
  <h2>Relatório Global — ${ini} a ${fim}</h2>
  <div class="footer">Gerado em ${new Date().toLocaleString("pt-BR")} — DeliveryBot</div>
  ${el.innerHTML.replace(/<canvas[^>]*>.*?<\/canvas>/gs, "").replace(/<button[^>]*>.*?<\/button>/gs, "")}
  </body></html>`;
...
function applySkin(id) {
  ...
  // Se tiver cores customizadas, aplicar por cima
  const custom = localStorage.getItem("dbCustomColors");
  if (custom) {
    try {
...
function applyCustomColors(c) {
  ...
  if (c.accent) root.style.setProperty("--ac", c.accent);
  if (c.accent2) root.style.setProperty("--ac2", c.accent2);
  if (c.bg) root.style.setProperty("--bg", c.bg);
  if (c.bg2) root.style.setProperty("--bg2", c.bg2);
  if (c.success) root.style.setProperty("--gr", c.success);
  if (c.error) root.style.setProperty("--rd", c.error);
  if (c.chart) root.style.setProperty("--bl", c.chart);
}
...
function loadSkinGrid() {
  ...
  SKINS.forEach(s => {
    ...
    div.onclick = () => {
      applySkin(s.id);
      localStorage.removeItem("dbCustomColors");
      document.documentElement.style.cssText = "";
      loadSkinGrid();
      initColorPickers();
      toast("t-skin","✅ Tema "+s.nome+" aplicado!",true);
    };
    div.innerHTML =
...
function initColorPickers() {
  ...
  const pickerConfigs = [
    { id: "accent", prop: "accent", fallback: "#ff4f1f" },
    { id: "bg", prop: "bg", fallback: "#090b10" },
    { id: "bg2", prop: "bg2", fallback: "#0f1219" },
    { id: "gr", prop: "success", fallback: "#1bcc6f" },
    { id: "rd", prop: "error", fallback: "#ff3351" },
    { id: "chart", prop: "chart", fallback: "#3d95ff" },
  ];

  pickerConfigs.forEach(cfg => {
    const picker = document.getElementById(`cor-${cfg.id}`);
    const hex = document.getElementById(`cor-${cfg.id}-hex`);
    if (picker) {
      picker.value = c[cfg.prop] || cfg.fallback;
      picker.oninput = () => {
    ...
    if (hex) {
      hex.value = c[cfg.prop] || cfg.fallback;
      hex.oninput = () => {
...
function updateColorPreview() {
  const accent = document.getElementById("cor-accent")?.value || "#ff4f1f";
  const success = document.getElementById("cor-gr")?.value || "#1bcc6f";
  const error = document.getElementById("cor-rd")?.value || "#ff3351";
  const chart = document.getElementById("cor-chart")?.value || "#3d95ff";

  const prevA = document.getElementById("prev-accent");
  const prevS = document.getElementById("prev-success");
  const prevE = document.getElementById("prev-error");
  const prevC = document.getElementById("prev-chart");

  if (prevA) prevA.style.background = accent;
  if (prevS) prevS.style.background = success;
  if (prevE) prevE.style.background = error;
  if (prevC) prevC.style.background = chart;
}

document.getElementById("btn-sv-cores")?.addEventListener("click", () => {
  const colors = {
    accent: document.getElementById("cor-accent")?.value || "#ff4f1f",
    accent2: document.getElementById("cor-accent")?.value || "#ff6e3f",
    bg: document.getElementById("cor-bg")?.value || "#090b10",
    bg2: document.getElementById("cor-bg2")?.value || "#0f1219",
    success: document.getElementById("cor-gr")?.value || "#1bcc6f",
    error: document.getElementById("cor-rd")?.value || "#ff3351",
    chart: document.getElementById("cor-chart")?.value || "#3d95ff",
  };

  localStorage.setItem("dbCustomColors", JSON.stringify(colors));
  applyCustomColors(colors);
  toast("t-cores", "✅ Cores personalizadas salvas!", true);
});

// Salvar configurações do WhatsApp
document.getElementById("sel-emoji-style")?.addEventListener("change", async function() {
  await api("POST", "/api/config", { emojiStyle: this.value });
  showToast("Estilo de emojis atualizado!", "ok");
});

document.getElementById("sel-msg-style")?.addEventListener("change", async function() {
  await api("POST", "/api/config", { msgStyle: this.value });
  showToast("Estilo das mensagens atualizado!", "ok");
});

(function(){
  applySkin(localStorage.getItem("dbSkin") || "carbon");
  // Aplicar cores customizadas se existirem
  const custom = localStorage.getItem("dbCustomColors");
  if (custom) { try { applyCustomColors(JSON.parse(custom)); } catch(_) {} }
})();
...
async function loadTTSConfig() {
  const r = await api("GET", "/api/tts/config");
  if (!r) return;
  
  document.getElementById("tog-tts").checked = r.enabled || false;
  document.getElementById("sel-tts-voice").value = r.voiceId || "21m00Tcm4TlvDq8ikWAM";
  document.getElementById("sel-tts-model").value = r.model || "eleven_multilingual_v2";
  document.getElementById("rng-tts-stability").value = r.stability || 0.5;
  document.getElementById("rng-tts-similarity").value = r.similarityBoost || 0.75;
  document.getElementById("rng-tts-style").value = r.styleExaggeration || 0.0;
  
  document.getElementById("val-tts-stability").textContent = r.stability || 0.5;
  document.getElementById("val-tts-similarity").textContent = r.similarityBoost || 0.75;
  document.getElementById("val-tts-style").textContent = r.styleExaggeration || 0.0;
  
  const statusEl = document.getElementById("tts-status");
  if (r.apiKeyConfigured) {
    statusEl.textContent = "✅ Chave configurada";
    statusEl.style.color = "var(--gr)";
  } else {
    statusEl.textContent = "⚠️ Chave não configurada";
    statusEl.style.color = "var(--am)";
  }
}

// Event listeners para TTS
document.getElementById("btn-sv-tts-key")?.addEventListener("click", async () => {
  const key = document.getElementById("inp-tts-key").value.trim();
  if (!key) { toast("t-tts-key", "❌ Digite a chave!", false); return; }
  const r = await api("POST", "/api/tts/key", { key });
  if (r?.ok) { toast("t-tts-key", "✅ Chave salva!", true); loadTTSConfig(); }
  else toast("t-tts-key", "❌ Erro: " + (r?.erro || "Desconhecido"), false);
});

document.getElementById("btn-sv-tts")?.addEventListener("click", async () => {
  const config = {
    enabled: document.getElementById("tog-tts").checked,
    voiceId: document.getElementById("sel-tts-voice").value,
    model: document.getElementById("sel-tts-model").value,
    stability: parseFloat(document.getElementById("rng-tts-stability").value),
    similarityBoost: parseFloat(document.getElementById("rng-tts-similarity").value),
    styleExaggeration: parseFloat(document.getElementById("rng-tts-style").value)
  };
  const r = await api("POST", "/api/tts/config", config);
  if (r?.ok) toast("t-tts", "✅ Configuração salva!", true);
  else toast("t-tts", "❌ Erro: " + (r?.erro || "Desconhecido"), false);
});

// Atualizar valores dos sliders em tempo real
["stability", "similarity", "style"].forEach(prop => {
  const slider = document.getElementById(`rng-tts-${prop}`);
  const display = document.getElementById(`val-tts-${prop}`);
  if (slider && display) {
...
document.getElementById("btn-test-tts")?.addEventListener("click", async () => {
  const text = document.getElementById("txt-tts-test").value.trim();
  if (!text) { toast("t-tts-test", "❌ Digite um texto!", false); return; }
  const engine = document.getElementById("sel-tts-engine")?.value || "google";
  
  try {
    toast("t-tts-test", "⏳ Gerando áudio...", true);
    const r = await api("POST", "/api/tts/test", { text, engine });
    if (r?.ok && r.audioUrl) {
      const audioEl = document.getElementById("tts-audio");
      audioEl.src = r.audioUrl;
      document.getElementById("tts-audio-player").style.display = "block";
      audioEl.play();
      toast("t-tts-test", "✅ Áudio gerado via " + (r.engine === "google" ? "Google TTS (grátis)" : "ElevenLabs") + "!", true);
    } else {
      toast("t-tts-test", "❌ " + (r?.erro || "Erro ao gerar áudio"), false);
    }
  } catch (e) {
    toast("t-tts-test", "❌ Erro: " + e.message, false);
  }
});
...
async function loadDashboardCharts() {
  try {
    ...
    // Usar cores customizadas se existirem
    const custom = localStorage.getItem("dbCustomColors");
    let c = {};
    if (custom) { try { c = JSON.parse(custom); } catch(_) {} }
    const chartColor = c.chart || "#3d95ff";
    ...
    if (data.hoje && data.hoje.pedidos) {
      data.hoje.pedidos.forEach(p => {
        const h = new Date(p.confirmado || p.inicio || p.data + "T" + p.hora).getHours();
        if (!isNaN(h)) vendasPorHora[h]++;
      });
    }
    ...
    if (ctxHoras) {
      ...
      _chartHoras = new Chart(ctxHoras, {
        type: "bar",
        data: {
          labels: horas,
          datasets: [{
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      dias.push(d.toLocaleDateString("pt-BR", { weekday: "short", day: "2-digit" }));
      const diaStr = d.toISOString().slice(0, 10);
      const count = (data.geral && data.geral.porDia || []).find(h => h.data === diaStr);
      vendasPorDia.push(count ? count.qtd : 0);
    }
...
window.exportarCSV = async function() {
  try {
    const data = await api("GET", "/api/relatorios");
    if (!data || !data.hoje || !data.hoje.pedidos || !data.hoje.pedidos.length) {
      showToast("Nenhum pedido para exportar!", "er");
      return;
    }
    
    const headers = ["Pedido", "Cliente", "Itens", "Pagamento", "Endereço", "Hora", "Data"];
    const rows = data.hoje.pedidos.map(p => [
      p.numPedido || "",
      (p.numero || "").replace("@c.us", ""),
      (p.itens || "").replace(/"/g, '""'),
      p.pagamento || "",
    ...
    showToast("Erro ao exportar: " + e.message, "er");
  }
};
...
window.exportarPDF = async function() {
  try {
    const data = await api("GET", "/api/relatorios");
    if (!data) { showToast("Erro ao carregar dados!", "er"); return; }
    
    const empresa = document.getElementById("e-nome")?.value || "Delivery";
    const hoje = new Date().toLocaleDateString("pt-BR");
    const pedidos = data.hoje?.pedidos || [];
    ...
    let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
      *{margin:0;padding:0;box-sizing:border-box}
      ...
      .stat{flex:1;min-width:120px;padding:12px;background:#f5f5f5;border-radius:8px;text-align:center}
      .stat-val{font-size:20px;font-weight:bold;color:#ff4f1f}
    ...
    <h1>${esc(empresa)}</h1>
    <h2>Relatório de Vendas - ${hoje}</h2>
    <div class="stats">
      <div class="stat"><div class="stat-val">${data.hoje?.qtd || 0}</div><div class="stat-lbl">Pedidos Hoje</div></div>
      <div class="stat"><div class="stat-val">R$ ${(data.hoje?.faturamento || 0).toFixed(2)}</div><div class="stat-lbl">Faturamento</div></div>
      <div class="stat"><div class="stat-val">${data.geral?.totalPedidos || 0}</div><div class="stat-lbl">Total Geral</div></div>
    </div>`;
    
    if (pedidos.length) {
      html += `<table><tr><th>#</th><th>Cliente</th><th>Itens</th><th>Pagamento</th><th>Hora</th></tr>`;
      pedidos.forEach(p => {
        html += `<tr><td>${p.numPedido||""}</td><td>${(p.numero||"").replace("@c.us","")}</td><td>${esc(p.itens||"")}</td><td>${esc(p.pagamento||"")}</td><td>${p.hora||""}</td></tr>`;
      });
      html += `</table>`;
    }
    
    html += `<div class="footer">Gerado em ${new Date().toLocaleString("pt-BR")} — DeliveryBot</div></body></html>`;
...
async function loadPesquisaConfig() {
  const r = await api("GET", "/api/pesquisa/config");
  if (!r) return;

  document.getElementById("tog-pesquisa").checked = r.ativo !== false;
  document.getElementById("pesq-tempo").value = r.tempoMinutos || 30;
  _pesqPerguntas = r.perguntas || [];
  renderPerguntas();
  loadPesquisaResultados();
}

function renderPerguntas() {
  const el = document.getElementById("pesq-perguntas");
  if (!el) return;
  if (!_pesqPerguntas.length) {
    el.innerHTML = '<div class="fe">Nenhuma pergunta configurada.</div>';
    return;
  }
  el.innerHTML = _pesqPerguntas.map((p, i) => `
    <div style="display:flex;align-items:center;gap:8px;padding:10px 12px;background:var(--bg3);border:1px solid var(--brd);border-radius:8px;margin-bottom:6px">
      <input class="inp" style="width:50px;text-align:center" value="${esc(p.emoji)}" maxlength="4"
        oninput="_pesqPerguntas[${i}].emoji=this.value"/>
      <input class="inp" style="flex:1" value="${esc(p.texto)}"
        oninput="_pesqPerguntas[${i}].texto=this.value" placeholder="Texto da pergunta"/>
      <button class="btn btd btsm" onclick="_pesqPerguntas.splice(${i},1);renderPerguntas()">🗑️</button>
    </div>
  `).join("");
}

document.getElementById("btn-add-pergunta")?.addEventListener("click", () => {
  _pesqPerguntas.push({ emoji: "⭐", texto: "Nova pergunta" });
  renderPerguntas();
});

document.getElementById("btn-sv-pesq")?.addEventListener("click", async () => {
  const ativo = document.getElementById("tog-pesquisa").checked;
  const tempo = parseInt(document.getElementById("pesq-tempo").value) || 30;
  const r = await api("POST", "/api/pesquisa/config", {
    ativo,
    tempoMinutos: tempo,
    perguntas: _pesqPerguntas
  });
  toast("t-pesq", r?.ok ? "✅ Configuração salva!" : "❌ Erro ao salvar.", !!r?.ok);
});

async function loadPesquisaResultados() {
  const r = await api("GET", "/api/pesquisa/resultados");
  if (!r) return;

  setEl("pesq-total", "textContent", r.total || 0);
  setEl("pesq-media", "textContent", r.mediaGeral || 0);
  setEl("pesq-hoje", "textContent", r.hoje || 0);

  // Distribuição
  const distEl = document.getElementById("pesq-distribuicao");
  if (distEl) {
    const total = r.total || 1;
    const cores = ["#ff3351", "#ff6e3f", "#f59e0b", "#1bcc6f", "#0ea5e9"];
    const labels = ["1⭐ Ruim", "2⭐ Regular", "3⭐ Ok", "4⭐ Bom", "5⭐ Excelente"];
    distEl.innerHTML = [1,2,3,4,5].map(n => {
      const qtd = r.distribuicao?.[n] || 0;
      const pct = Math.round((qtd / total) * 100);
      return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
        <span style="font-size:.78rem;min-width:90px;color:var(--mu)">${labels[n-1]}</span>
        <div style="flex:1;height:18px;background:var(--bg4);border-radius:4px;overflow:hidden">
          <div style="height:100%;width:${pct}%;background:${cores[n-1]};border-radius:4px;transition:width .5s"></div>
        </div>
        <span style="font-size:.76rem;min-width:35px;text-align:right;color:var(--tx)">${qtd}</span>
      </div>`;
    }).join("");
  }

  // Últimas pesquisas
  const ultEl = document.getElementById("pesq-ultimas");
  if (ultEl) {
    if (!r.ultimas?.length) {
      ultEl.innerHTML = '<div class="fe">Nenhuma pesquisa recebida ainda.</div>';
    } else {
      ultEl.innerHTML = r.ultimas.map(p => {
        const estrelas = "⭐".repeat(Math.round(p.avaliacaoMedia || p.avaliacao || 0));
        const dataFmt = p.data ? new Date(p.data+"T12:00").toLocaleDateString("pt-BR") : "";
        return `<div style="padding:8px 12px;background:var(--bg3);border-radius:8px;margin-bottom:4px;display:flex;align-items:center;gap:10px">
          <span style="font-size:1rem">${estrelas}</span>
          <div style="flex:1">
            <div style="font-weight:600;font-size:.84rem">#${esc(p.numPedido)}</div>
            <div style="font-size:.72rem;color:var(--mu)">${dataFmt} ${p.hora||""}</div>
          </div>
          <span style="font-family:var(--fh);font-weight:800;font-size:1rem;color:var(--ac)">${p.avaliacaoMedia || p.avaliacao || 0}/5</span>
        </div>`;
      }).join("");
    }
  }
...
document.getElementById("btn-at-pesq")?.addEventListener("click", loadPesquisaResultados);
