...
app.get("/api/status", guard, (req, res) => {
  ...
  res.json({
    ...
    autoImprimir, viasImpressao, agenteAtivo,
...
app.post("/api/cardapio/item", guard, (req, res) => {
  const { categoria, nome, descricao, preco, tamanhos, maxAcomp } = req.body || {};
...
app.put("/api/cardapio/item", guard, (req, res) => {
  const { categoria, nomeOriginal, nome, descricao, preco, tamanhos, maxAcomp } = req.body || {};
  const cfg = loadConfig();
  const cat = (cfg.cardapio || []).find(c => c.categoria === categoria);
...
function querCancelar(t) {
  return ["cancelar","cancela","desistir","nao quero","não quero","sair"].some(g => t.toLowerCase().includes(g));
}
function querVoltarMenu(t) {
  const s = t.trim().toLowerCase();
  return ["menu","cardápio","cardapio","ver menu","voltar ao menu","voltar menu","início","inicio"].some(g => s === g || s.startsWith(g + " "));
}
...
function resolverPag(txt, cfg) {
  ...
  return formas.find(f => txt.toLowerCase().includes(f.toLowerCase())) || null;
}
...
// Extrai quantidade e nome do item — suporta:
// "2x X-Bacon", "2 X-Bacon", "dois X-Bacon", "X-Bacon duplo", "2 coxinha duplo"
function extrairQtd(txt) {
  const original = txt.trim();
  const lower = original.toLowerCase();

  // Padrão 1: número no início seguido de "x" (2x X-Bacon, 2 x-salada)
  const mNumX = lower.match(/^(\d+)\s*[xX]?\s+(.+)$/);
  if (mNumX) return { qtd: Math.min(parseInt(mNumX[1]), 20), texto: mNumX[2].trim() };

  // Padrão 2: número por extenso no início (dois X-Bacon, uma pizza, tres cocas)
  const partes = lower.split(/\s+/);
  const primeira = partes[0];
  if (NUMEROS_EXTENSO[primeira] !== undefined) {
    const num = NUMEROS_EXTENSO[primeira];
    let resto = partes.slice(1).join(" ");
    // Remove "x" opcional entre número e item
    resto = resto.replace(/^x\s+/, "").trim();
    if (resto.length > 0) {
      return { qtd: Math.min(num, 20), texto: resto };
    }
  }

  // Padrão 3: "X-Bacon duplo" ou "X-Bacon 2" — duplo = 2, 2 = multiplica quantidade
  const mDuplo = lower.match(/^(.+?)\s+(duplo|dupla|2\s*$)$/i);
  if (mDuplo) {
    const item = mDuplo[1].trim();
    // Verificar se o item começa com quantidade (ex: "2 coxinha duplo" = 4)
    const mItemNum = item.match(/^(\d+)\s+(.+)$/);
    if (mItemNum) {
      const qtd = Math.min(parseInt(mItemNum[1]) * 2, 20);
      return { qtd, texto: mItemNum[2].trim() };
    }
    return { qtd: 2, texto: item };
  }

  // Padrão 4: "meia pizza" ou "meia marguerita" — meia = 1/2
  const mMeia = lower.match(/^meia?\s+(.+)$/i);
  if (mMeia) return { qtd: 1, texto: "1/2 " + mMeia[1].trim(), ehMeia: true };

  // Sem quantidade específica — retorna 1
  return { qtd: 1, texto: original };
}
...
async function handleMsg(msg) {
  try {
    ...
    if (!txt && !isLoc) {
      if (msg.hasMedia || msg.type === "ptt" || msg.type === "audio") {
        ...
        if (msg.type === "ptt" || msg.type === "audio") {
          await msg.reply(
            `🎙️ Recebi seu áudio, mas não consigo ouvir! 😅\n\n` +
    ...
    if (querAtendente || ehReclam) {
      if (atendHumanoAtivo && !atendimentoHumano.has(numero)) {
        // Pausar bot e abrir atendimento humano
      ...
      } else if (!atendHumanoAtivo) {
        const tel = cfg.empresaTelefone||"";
        const numAtend2 = (cfg.numeroAtendente||"").replace(/\D/g,"");
        const linkDireto = numAtend2
          ? `\n\n📲 Fale diretamente: https://wa.me/55${numAtend2}`
          : tel ? `\n\n📞 Ligue: *${tel}*` : "";
        if (ehReclam) {
          resp = (
            `😔 *Sentimos muito pelo ocorrido!*\n\n` +
            `Sua mensagem foi registrada e nossa equipe vai verificar o que aconteceu.\n` +
            `Pedimos desculpas pelo transtorno! 🙏${linkDireto}\n\n` +
            `Digite *atendente* a qualquer momento se quiser falar com a equipe.`
          );
        } else {
          resp = (
            `👤 No momento o atendimento humano está desativado.\n` +
            `Posso te ajudar aqui mesmo! 😊${linkDireto}\n\n` +
            `Me conta o que precisa.`
          );
        }
      } else {
        // Já está em atendimento
        resp = `👤 Você já está sendo atendido por um de nossos atendentes. Aguarde! 😊`;
      }
      await responderHumano(chat, msg, resp);
      logC({ tipo:"saida", para:numero, mensagem:resp });
      io.emit("msg", { numero, mensagem:resp, tipo:"saida" });
    ...
    // ════════════════════════════════════════════════════
    if (!estadosPedido.has(numero)) {
      const sLowP = txt.trim().toLowerCase();
      const querPromo = ["7","promo","promoção","promocao","promoções","promocoes",
        "oferta","ofertas","desconto","especial","novidade"].some(g => sLowP === g || sLowP === g.trim());
      if (querPromo) {
    ...
    if (fl) {
      ...
      // Se reclamação detectada E recurso ativo → oferecer atendente
      if (ehReclam && atendHumanoAtivo && !atendimentoHumano.has(numero)) {
        resp += (
          `\n\n❓ *Quer falar com um atendente humano?*\n` +
          `Digite *atendente* e um de nós assume o chat agora.`
        );
      }
