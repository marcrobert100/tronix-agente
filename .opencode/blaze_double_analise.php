...
class BlazeDoubleSimulator {
    ...
    public function runAnalysis(): void {
        echo "==========================================\n";
        echo "    BLAZE DOUBLE - ANÁLISE ESTATÍSTICA\n";
        echo "==========================================\n\n";
        
        echo "📊 TABELA DE FREQUÊNCIAS (1.000 rodadas)\n";
        echo "----------------------------------------\n";
        ...
        echo "📈 ANÁLISE DE SEQUÊNCIAS\n";
        echo "----------------------------------------\n";
        $streaks = $this->getStreaks();
        foreach ($streaks as $color => $data) {
            $name = $color === 'W' ? 'White' : ($color === 'R' ? 'Red' : 'Black');
            echo "{$name}: Máx={$data['max']}, Média={$data['avg']}\n";
        }
        
        echo "\n📉 TESTE QUI-QUADRADO\n";
        echo "----------------------------------------\n";
        $chiSquare = $this->getChiSquare($expected);
        echo "Valor χ²: {$chiSquare}\n";
        echo "Valor crítico (p=0.05, gl=2): 5.991\n";
        
        if ($chiSquare < 5.991) {
            echo "✅ Distribuição uniforme confirmada\n";
        } else {
            echo "⚠️ Desvio significativo detectado\n";
        }
