...
def _get_transcript(video_id: str) -> str | None:
    ...
    try:
        ...
        lang_priority = ["pt-BR", "pt", "en", "tr", "de", "fr", "es", "it", "ru", "ja", "ko", "ar", "zh"]

        def score(t):
            code = getattr(t, "language_code", "").lower()
            base = lang_priority.index(code) if code in lang_priority else len(lang_priority)
            return (0 if getattr(t, "is_generated", False) is False else 1, base)

        transcript = None
        try:
            transcript = min(transcript_list, key=score)
        except ValueError:
            return None
...
def _summarize_with_gemini(transcript: str, video_url: str) -> str:
    ...
    response  = _client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=(
                "You are TRONIX, an AI assistant of Marcos Roberto at PCsoluções. "
...
def _scrape_trending(region: str = "BR", max_results: int = 8) -> list[dict]:
    if not _REQUESTS_OK:
        return []

    # feed de trending e bloqueado por bot-wall; rota estavel = busca filtrada (so videos)
    query = "trending top videos" if region.upper() == "EN" else "em alta hoje brasil"
    search_url = (
        f"https://www.youtube.com/results"
    ...
    try:
        ...
        blocks = html.split('"videoRenderer":{"videoId":"')[1:]
        results, seen = [], set()
        for block in blocks:
            video_id = block[: block.index('"')]
            if video_id in seen:
                continue

            title_m = re.search(r'"title":\{"runs":\[\{"text":"([^"]+)"', block)
            chan_m  = re.search(r'"ownerText":\{"runs":\[\{"text":"([^"]+)"', block)
            meta_m  = re.search(r'"viewCountText":\{"simpleText":"([^"]+)"', block)
            if not title_m:
                continue
            ...
            results.append({
                ...
                "views":   meta_m.group(1) if meta_m else "",
                "url":     f"https://www.youtube.com/watch?v={video_id}",
            })
...
def _handle_trending(parameters: dict, player, speak) -> str:
    region = parameters.get("region", "BR").upper()
